...
export default async function handler(req, res) {
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        html: `
          <div style="max-width:600px;margin:0 auto;font-family:Georgia,serif;background:#0a0a0a;color:#d4c9a8;padding:40px 30px;border:1px solid #2a2010;">
            ...
            <p style="color:#8a7a5a;font-size:15px;line-height:1.7;">A partir de ahora recibirÃ¡s reflexiones, ritos y enseÃ±anzas directamente en tu bandeja de entrada. Cada mensaje serÃ¡ un recordatorio para hacer una pausa, encender la conciencia y dejar que el humo te guÃ­e.</p>
            <div style="border-left:2px solid #c9a84c;padding:14px 20px;margin:24px 0;background:#0f0d08;">
              <p style="color:#a09070;font-style:italic;font-size:15px;line-height:1.7;">
                "El humo no miente. En su transparencia se refleja la verdad del que fuma."
              </p>
              <p style="color:#6a5a3a;font-size:13px;text-align:right;">â€” Ãšltimo Testimonio, 1:1</p>
            </div>
            <p style="color:#6a5a3a;font-size:13px;line-height:1.6;">Si en algÃºn momento deseas dejar de recibir estos mensajes, simplemente responde a este correo con "Darme de baja".</p>
            <hr style="border:none;border-top:1px solid #1a1408;margin:24px 0;">
            <p style="text-align:center;color:#3a3020;font-size:11px;letter-spacing:1px;">La Llama Interior &bull; Una filosofÃ­a de conciencia plena</p>
          </div>
        `,
      }),
    });
    ...
    console.log(` SuscripciÃ³n exitosa: ${email} â€” Resend ID: ${data.id}`);

    return res.status(200).json({
      message: 'SuscripciÃ³n exitosa. Revisa tu correo.',
      id: data.id,
    });
  } catch (error) {
